...
///-----------------------------------------------------------------
///
/// @file      CoulombAddDialog.cpp
/// @author    niraj
/// Created:   7/7/2012 12:31:35 PM
/// @section   DESCRIPTION
///            CoulombAddDialog class implementation
///
///------------------------------------------------------------------

#include "CoulombAddDialog.h"
...
////Event Table Start
BEGIN_EVENT_TABLE(CoulombAddDialog,wxDialog)
	////Manual Code Start
	////Manual Code End
	
	EVT_CLOSE(CoulombAddDialog::OnClose)
	EVT_CHECKBOX(ID_CUSTOMNAMECHECKBOX,CoulombAddDialog::CustomNameCheckBoxClick)
	EVT_BUTTON(ID_CANCELBUTTON,CoulombAddDialog::CancelButtonClick)
	EVT_BUTTON(ID_ADDBUTTON,CoulombAddDialog::AddButtonClick)
	EVT_COMBOBOX(ID_REFERENCEOBJECTCOMBOBOX,CoulombAddDialog::ReferenceObjectComboBoxSelected)
END_EVENT_TABLE()
////Event Table End

CoulombAddDialog::CoulombAddDialog(wxWindow *parent, wxArrayString (*getObjectNames)(void), wxWindowID id, const wxString &title, const wxPoint &position, const wxSize& size, long style)
...
CoulombAddDialog::~CoulombAddDialog()
{
} 

void CoulombAddDialog::CreateGUIControls()
{
	...
	MultipleChargeLabel = new wxStaticText(WxScrolledWindow1, ID_MULTIPLECHARGELABEL, _("Charge(Coulombs):"), wxPoint(135, 6), wxDefaultSize, 0, _("MultipleChargeLabel"));

	MultipleChargeTextbox = new wxTextCtrl(WxScrolledWindow1, ID_MULTIPLECHARGETEXTBOX, _("1"), wxPoint(141, 24), wxSize(100, 19), 0, wxDefaultValidator, _("MultipleChargeTextbox"));

	SingleChargeLabel = new wxStaticText(MultiplicitySingleNotebookPage, ID_SINGLECHARGELABEL, _("Charge(Coulombs):"), wxPoint(135, 6), wxDefaultSize, 0, _("SingleChargeLabel"));

	SingleChargeTextbox = new wxTextCtrl(MultiplicitySingleNotebookPage, ID_SINGLECHARGETEXTBOX, _("1"), wxPoint(141, 24), wxSize(100, 19), 0, wxDefaultValidator, _("SingleChargeTextbox"));

	SetTitle(_("CoulombAddDialog"));
	SetIcon(wxNullIcon);
	SetSize(8,8,339,387);
	Center();
...
void CoulombAddDialog::OnClose(wxCloseEvent& /*event*/)
{
	//Destroy();
	this->EndModal(0);
}

void CoulombAddDialog::BeforeShowing()
{
 ...
 * AddButtonClick
 */
void CoulombAddDialog::AddButtonClick(wxCommandEvent& event)
{
	objEnabled = EnabledCheckbox->IsChecked();
	if(ReferenceObjectComboBox->GetValue() == _("Absolute"))
	{
		//Absolute
		refObjType = CoulombReferenceObjectType_Absolute;
	}
	else
	{
		//Relative
		refObjType = CoulombReferenceObjectType_Relative;
	...
	if(MultiplicityNotebook->GetSelection() == 0)
	{
		...
		hasNoError = (MassSingleTextbox->GetValue().ToDouble(&singleMass) &&
			RadiusSingleTextbox->GetValue().ToDouble(&singleRadius) && SingleChargeTextbox->GetValue().ToDouble(&singleCharge) &&
	...
	else
	{
		...
		if (hasNoError)
		{
			...
			multipleCharge = MultipleChargeTextbox->GetValue();
 ...
 * CancelButtonClick
 */
void CoulombAddDialog::CancelButtonClick(wxCommandEvent& event)
{
	this->EndModal(0);
}
 ...
 */
void CoulombAddDialog::ReferenceObjectComboBoxSelected(wxCommandEvent& event )
{
 ...
 */
void CoulombAddDialog::CustomNameCheckBoxClick(wxCommandEvent& event)
{
